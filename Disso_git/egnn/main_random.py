...
for d, dataset in enumerate(datasets):
    ...
    for j in range(args.num_trials):
        ...
        # ************************************************************
        # We set conmpletely random node features
        # ************************************************************
        vals = []
        X = np.random.uniform(low=0.0,high=1.0,size=X.shape).astype(np.float32)
        ...
        #vals.append((sparse.as_coo(X) * rowsum).to_scipy_sparse() + sparse.as_coo(noise).to_scipy_sparse()) (adding noise after normalization)
        vals.append((sparse.as_coo(X) * rowsum).to_scipy_sparse()) #+ sparse.as_coo(noise).to_scipy_sparse())
        ...
        for a,alpha_val in enumerate(alpha_vals):
            ...
            with tf.Session() as sess:
                ...
                for epoch in range(args.epochs):
                    ...
                    test_accs[d,a,j,epoch,0] = loss_val_np
                    test_accs[d,a,j,epoch,1] = acc_test

                    np.save("pubmed_CS_random",test_accs)
