...
X,Y,idx_train,idx_test,idx_val,G = utils.load_data_lastfm()
num_nodes = len(G.nodes)
A = np.random.uniform(size=(num_nodes,num_nodes))
print("IDX TRAIN LENGTH: ", len(idx_train))
print("IDX TRAIN 3rd ELEM: ", idx_train[2])
print("IDX TRAIN 3rd ELEM TYPE: ", type(idx_train[2]))


print(Y)
K = A.shape[1] if X is None else A.shape[1]
nC = Y.shape[1]
...
#G = utils.load_graph('data_lasftm_asia/lastfm_asia_edges.csv')
ollivier_curv_vals, forman_curv_vals = csr_matrix((num_nodes,num_nodes)).toarray(), csr_matrix((num_nodes,num_nodes)).toarray()
...
print("type vals pre stack: ", type(vals))
vals = sparse.stack(vals, axis=0)
print("type vals post stack: ", type(vals))
vals = vals.todense()
print("type vals post todense: ", type(vals))
